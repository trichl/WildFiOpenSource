#ifndef PlatformWildFiTagREV5_h
#define PlatformWildFiTagREV5_h
...
class WildFiTagREV5; // forward declaration for post_task_stream_flash_parameters_t
...
/*#define ESP_NOW_STREAM_PAYLOAD_LEN						247
#define ESP_NOW_STREAM_PAYLOAD_LAST_MESSAGE_LEN			(2048 - (8 * ESP_NOW_STREAM_PAYLOAD_LEN))*/

/** ESPNOW Stream Proximity + GPS */
#define ESP_NOW_FLASH_STREAM_PROXIMITY_UNLIMITED				0
#define ESP_NOW_FLASH_STREAM_PROXIMITY_HEADER_LEN				7
#define ESP_NOW_FLASH_STREAM_PROXIMITY_PAYLOAD_LEN				(250 - ESP_NOW_FLASH_STREAM_PROXIMITY_HEADER_LEN)
//#define ESP_NOW_FLASH_STREAM_RETRIES_WAIT_BETWEEN_MS			100
...
typedef struct {
    ...
    const char *postfix;
	uint8_t *dataDMA2048Bytes;						// DMA allocated buffer for flash reads
	...
	WildFiTagREV5 *deviceObject;					// needed for callback to readSupplyVoltage() function -> evaluating if we should continue to send data
...
class WildFiTagREV5 {
	public:
		...
		void doWiFiPOSTStreamCallFlash(post_task_stream_flash_parameters_t *globalPostParameters, const uint16_t taskStackSize = 8192); // TURN FLASH ON BEFORE THAT, monster function: reading FIFO from flash, transmitting it as a http POST stream
		void doWiFiPOSTStreamCallFlashFullBlock(post_task_stream_flash_parameters_t *globalPostParameters, const uint16_t taskStackSize);
		...
		/** WildFiTagREV5 integrated modules */
		RTC_RV3028C7 rtc = RTC_RV3028C7();
