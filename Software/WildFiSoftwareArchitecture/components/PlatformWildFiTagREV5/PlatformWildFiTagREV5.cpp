...
static void restPostStreamFlashTask(void *pvParameters);
...
bool WildFiTagREV5::selfTest(uint16_t voltageSupplied, uint32_t testBits) {
	bool isAlive = false;
	bool error = false;
	const uint32_t testTimestamp = 1605269136;
	...
	if((testBits & SELFTEST_I2C) == 0) { printf("..SKIPPED\n"); }
		...
		isAlive = false;
		isAlive = i2c.isAlive(RTC_RV3028C7_ADDRESS);
	...
	if((testBits & SELFTEST_RTC) == 0) { printf("..SKIPPED\n"); }
		...
		if(!rtc.setTimestamp(testTimestamp)) { printf("..FAILED (set)\n"); sensorPowerOff(); return false; }
		uint32_t timestamp = rtc.getTimestamp(error);
		if(error) { printf("..FAILED (get)\n"); return false; }
		if(timestamp != testTimestamp) { printf("..FAILED (not equal)\n"); sensorPowerOff(); return false; }
		delay(1200);
		timestamp = rtc.getTimestamp(error);
		if(timestamp != testTimestamp + 1) { printf("..FAILED (not +1)\n"); sensorPowerOff(); return false; }
		printf("..OKAY\n");
	}
...
uint32_t WildFiTagREV5::measureTime(const char* text, bool dontPrint) {
	uint32_t duration;
...
/*bool WildFiTagREV5::customPhyInit() {
...
bool WildFiTagREV5::fullRFCalibration() {
    // takes 145 - 160ms
    const esp_phy_init_data_t* init_data = esp_phy_get_init_data();
    if(init_data == NULL) {
        return false;
...
bool WildFiTagREV5::onlyLoadRFCalibration() {
    // takes 5ms
    const esp_phy_init_data_t* init_data = esp_phy_get_init_data();
    if(init_data == NULL) {
        return false;
...
esp_reset_reason_t WildFiTagREV5::getLastResetReason() {
	return esp_reset_reason();
}

bool WildFiTagREV5::enableDynamicFrequencyScaling() {
...
bool WildFiTagREV5::flashPowerOn(bool withDelay) {
...
bool WildFiTagREV5::flashPowerOff(bool withDelay) {
	bool ret = true;
	...
	io_conf.mode = GPIO_MODE_DISABLE; // fully disable
...
void WildFiTagREV5::sensorPowerOn() {
...
void WildFiTagREV5::sensorPowerOff() {
...
void WildFiTagREV5::keepSensorPowerOnInDeepSleep() {
...
void WildFiTagREV5::delay(uint16_t d) {
	Timing::delay(d);
}

void WildFiTagREV5::preciseDelay(uint32_t us) {
	ets_delay_us(us);
}

void WildFiTagREV5::deepSleep(bool leavePowerOn) {
...
void WildFiTagREV5::shortLightSleep(uint32_t millseconds) {
...
uint32_t WildFiTagREV5::readSupplyVoltageFromADC() {
...
uint32_t WildFiTagREV5::readSupplyVoltageFromWakeStub() {
	return adcValue;
}

uint16_t WildFiTagREV5::readSupplyVoltage() {
	uint32_t voltage;
	if(!ADCinitialized) { // requires initPins before read
		return 0;
	}
...
int32_t WildFiTagREV5::readHallSensor(uint16_t iterations) {
	adc1_config_width(ADC_WIDTH_BIT_12); // 12 bit width
    int32_t val = 0;
...
bool WildFiTagREV5::printSpiffs(const char* fileName) {
	uint16_t cntLines = 0;
...
void WildFiTagREV5::resetSpiffs(const char* fileName) {
    struct stat st;
...
uint32_t WildFiTagREV5::getFreeSpaceSpiffs() {
...
void WildFiTagREV5::wasteOn() {
	ledGreenOn();
	ledRedOn();
}

void WildFiTagREV5::wasteOff() {
	ledGreenOff();
	ledRedOff();
}

void WildFiTagREV5::ledRedOn() {
...
void WildFiTagREV5::ledRedOff() {
	if(gpioHoldEnabled) { gpio_hold_dis(PIN_LED_RED); } // when keepSensorPowerOnInDeepSleep was called
	gpio_set_level(PIN_LED_RED, 0);
	if(gpioHoldEnabled) { gpio_hold_en(PIN_LED_RED); } // when keepSensorPowerOnInDeepSleep was called
}

void WildFiTagREV5::ledGreenOn() {
	gpio_set_level(PIN_LED_GREEN, 1);
}

void WildFiTagREV5::ledGreenOff() {
	gpio_set_level(PIN_LED_GREEN, 0);
}

void WildFiTagREV5::blinkTimes(uint8_t howOften, blink_t color, bool addDelayAtEnd) {
...
void WildFiTagREV5::blink(blink_t l1, blink_t l2, blink_t l3) {
	// off
	ledRedOff();
	ledGreenOff();
	// first blink
...
wake_up_reason_t WildFiTagREV5::getWakeUpReason() {
	/*
...
void WildFiTagREV5::enableInternalTimerInterruptInDeepSleep(uint32_t seconds) {
	// max. 400 days
...
void WildFiTagREV5::printTxPower() {
    int8_t txPwr = 0;
	esp_wifi_get_max_tx_power(&txPwr);
	printf("TX POWER: %d\n", txPwr);
}

void WildFiTagREV5::gpioBOn() {
...
QueueHandle_t* WildFiTagREV5::uart2GetQueue() {
	return &uart2Queue;
}

bool WildFiTagREV5::uart2Init(uint32_t baudRate) {
...
void WildFiTagREV5::uart2UpdateBaudrate(uint32_t baudRate) {
	uart_set_baudrate(UART2_PORT_NUMBER, baudRate);
	uart_flush(UART2_PORT_NUMBER);
}

void WildFiTagREV5::uart2EnablePatternInterrupt(char pattern) {
...
void WildFiTagREV5::uart2InterruptPrint(uint16_t stopAfter) {
...
bool WildFiTagREV5::initDataNVS() {
...
bool WildFiTagREV5::resetDataNVS() {
	if(nvs_flash_erase_partition(NVS_DATA_PARTITION) != ESP_OK) {
		return false;
	}
	return true;
...
void WildFiTagREV5::printDataNVSStats() {
...
uint8_t WildFiTagREV5::nvsReadUINT8(const char *key) {
	esp_err_t err;
...
uint16_t WildFiTagREV5::nvsReadUINT16(const char *key) {
	esp_err_t err;
...
uint32_t WildFiTagREV5::nvsReadUINT32(const char *key) {
	esp_err_t err;
...
bool WildFiTagREV5::nvsWriteUINT8(const char *key, uint8_t val) {
	esp_err_t err;
	if(!NVSForDataInitialized) {
		return false;
	}
...
bool WildFiTagREV5::nvsWriteUINT16(const char *key, uint16_t val) {
	esp_err_t err;
	if(!NVSForDataInitialized) {
		return false;
	}
...
bool WildFiTagREV5::nvsWriteUINT32(const char *key, uint32_t val) {
	esp_err_t err;
	if(!NVSForDataInitialized) {
		return false;
	}
...
bool WildFiTagREV5::nvsWriteUINT32x2(const char *key1, uint32_t val1, const char *key2, uint32_t val2) {
	esp_err_t err;
	if(!NVSForDataInitialized) {
		return false;
	}
...
bool WildFiTagREV5::nvsWriteUINT32andUINT16(const char *key1, uint32_t val1, const char *key2, uint16_t val2) {
	esp_err_t err;
	if(!NVSForDataInitialized) {
		return false;
	}
...
uint32_t WildFiTagREV5::nvsReadThenPlusOneUINT32(const char *key) {
	esp_err_t err;
	uint32_t value = 0;
	if(!NVSForDataInitialized) {
		return false;
...
bool WildFiTagREV5::initESPNOWStationary(bool longrange, int8_t maxTxPower, bool withoutNVS, wifi_phy_rate_t phyRate) {
...
bool WildFiTagREV5::initESPNOW(bool longrange, int8_t maxTxPower, bool withoutNVS, wifi_phy_rate_t phyRate) {
	esp_log_level_set("ESPNOW", ESP_LOG_NONE); // saves 1ms
...
bool WildFiTagREV5::addESPNOWBroadcastReceiver() {
...
bool WildFiTagREV5::addESPNOWBroadcastReceiverStationary() {
	esp_now_peer_info_t peerInfo;
	if(!espNowInitialized) {
		return false;
	}
...
bool WildFiTagREV5::addESPNOWReceiver(uint8_t *mac) {
	esp_now_peer_info_t peerInfo;
	if(!espNowInitialized) {
		return false;
	}
...
bool WildFiTagREV5::addESPNOWReceiverStationary(uint8_t *mac) {
	esp_now_peer_info_t peerInfo;
	if(!espNowInitialized) {
		return false;
	}
...
esp_now_sending_status_t WildFiTagREV5::getESPNOWSendingStatus() {
	return espNowSendingStatus;
}

bool WildFiTagREV5::broadcastESPNOWData(uint8_t *data, uint8_t len) {
...
bool WildFiTagREV5::sendESPNOWData(uint8_t *data, uint8_t len) {
	if(len > ESP_NOW_MAX_DATA_LEN) {
		return false;
	}
...
bool WildFiTagREV5::sendESPNOWDataToMac(uint8_t *mac, uint8_t *data, uint8_t len) {
	if(len > ESP_NOW_MAX_DATA_LEN) {
		return false;
	}
	espNowSendingStatus = ESP_NOW_NOT_FINISHED; // for ACK
...
bool WildFiTagREV5::connectToWiFiAfterScan(const char* ssid, const char* password, uint8_t channel) {
...
bool WildFiTagREV5::wiFiSetCountryToUseChannel1to11() {
...
bool WildFiTagREV5::connectToWiFiDirectly(const char* ssid, const char* password, int8_t maxTxPower, uint8_t channel) {
	if(!wiFiInitialized) {
		return false;
	}
...
void WildFiTagREV5::disconnectAndStopWiFi() {
...
bool WildFiTagREV5::scanForWiFisOn1and6and11WithPriority(bool debug, const char** ssids, const uint8_t wifiListSize, uint8_t *wifiArrayId, uint8_t *foundOnChannel, int8_t maxTxPower, uint32_t scanTimePerChannel1and6, uint16_t timeoutScanMs) {
...
bool WildFiTagREV5::scanForWiFisOn1and6and11and13WithPriority(bool debug, const char** ssids, const uint8_t wifiListSize, uint8_t *wifiArrayId, uint8_t *foundOnChannel, int8_t maxTxPower, uint32_t scanTimePerChannel1and6, uint16_t timeoutScanMs) {
	// PROBLEM: if scanTime != 120 and wifi on channel 6 or 1 -> will not be changed back to 120s!!!
...
bool WildFiTagREV5::scanForWiFisOn1and6and11(const char** ssids, const uint8_t wifiListSize, uint8_t *wifiArrayId, uint8_t *foundOnChannel, int8_t maxTxPower, uint32_t scanTimePerChannel1and6, uint16_t timeoutScanMs) {
	// PROBLEM: if scanTime != 120 and wifi on channel 6 or 1 -> will not be changed back to 120s!!!
	const uint8_t CHANNEL_LIST[] = { 6, 1, 11 }; // scan first on channel 6 because highest probability
	uint8_t CHANNEL_LIST_SIZE = sizeof(CHANNEL_LIST) / sizeof(CHANNEL_LIST[0]);
...
bool WildFiTagREV5::scanForWiFis(bool blocking, int8_t maxTxPower, uint32_t scanTimePerChannel, uint8_t channel) {
	if(!wiFiInitialized) {
		return false;
	}
	if(channel == WIFI_CHANNELS_1_TO_11) {
...
bool WildFiTagREV5::wiFiScanCompleted() {
	return wiFiScanDone;
}

wifi_connect_status_t WildFiTagREV5::connectedToWiFi() {
	return wiFiConnected;
}

bool WildFiTagREV5::wiFiScanIncludesArray(const char** ssids, const uint8_t wifiListSize, uint8_t *wifiArrayId, uint8_t *foundChannel) {
...
bool WildFiTagREV5::wiFiScanIncludes(const char* ssid, uint8_t *foundChannel) {
    uint16_t number = DEFAULT_SCAN_LIST_SIZE;
    wifi_ap_record_t ap_info[DEFAULT_SCAN_LIST_SIZE];
    uint16_t ap_count = 0;
    memset(ap_info, 0, sizeof(ap_info));
...
uint8_t WildFiTagREV5::printWiFiScanResults() {
    uint16_t number = DEFAULT_SCAN_LIST_SIZE;
    wifi_ap_record_t ap_info[DEFAULT_SCAN_LIST_SIZE];
    uint16_t ap_count = 0;
    memset(ap_info, 0, sizeof(ap_info));
...
void WildFiTagREV5::restPostStreamGetSuccessfullyTransmittedBlocks(uint16_t &wiFiPostStreamBlocksSuccessfullyTransmittedIn, uint16_t &wiFiPostStreamHalfBlocksSuccessfullyTransmittedIn) {
...
static void restPostStreamFlashTaskFullBlocks(void *pvParameters) {
	const uint32_t PAGES_IN_ONE_GO = 64; // 64 * 2048 = 131072
	...
	if(useBase64Encoding) { // malloc memory for encoded data
		base64SpaceNeeded = mallocBASE64Encoding(taskParams.dataDMA2048Bytes, MT29_CACHE_SIZE, &payloadData); // length includes 0-termination
	...
	for(uint16_t multipleBlockIterator=0; multipleBlockIterator<(taskParams.flashMaxNumberOfBlocksToTransmit); multipleBlockIterator++) {
		...
		for(uint32_t singleBlockIterator=0; singleBlockIterator<PAGES_IN_ONE_GO; singleBlockIterator++) {
			// read one flash page (2048 bytes) -> store in taskParams.dataDMA2048Bytes
			if(!((taskParams.flashObject)->read(currentFlashPage, 0, taskParams.dataDMA2048Bytes, MT29_CACHE_SIZE))) {
			...
			currentFlashPage++;
			if(useBase64Encoding) { 
				BASE64Encoding(taskParams.dataDMA2048Bytes, MT29_CACHE_SIZE, &payloadData, base64SpaceNeeded); // encode flash data
				...
				writeLen = esp_http_client_write(client, (char *) (taskParams.dataDMA2048Bytes), MT29_CACHE_SIZE); // write data, BAD INTERNET (Edge only) -> WILL HANG HERE LONG TIME
			}
			if(writeLen <= 0)  {
				if(taskParams.debug) { printf("POST: error, client write failed\n"); }
				wiFiPostDataStatus = HTTP_POST_DATA_ERROR_CLIENT_WRITE;
		...
		if(taskParams.debug) { printf("POST: SUCCESSFULLY TRANSMITTED one block in one transmission\n"); }
...
wifi_post_data_status_t WildFiTagREV5::getWiFiPOSTCallStatus() {
	return wiFiPostDataStatus;
}

uint16_t WildFiTagREV5::getWiFiPostReturnCode() {
	return wiFiPostReturnCode;
}

void WildFiTagREV5::doWiFiPOSTCall(post_task_parameters_t *globalPostParameters, const uint16_t taskStackSize) {
...
void WildFiTagREV5::doWiFiPOSTStreamCallFlash(post_task_stream_flash_parameters_t *globalPostParameters, const uint16_t taskStackSize) {
	if(wiFiConnected != WIFI_CONNECT_SUCCESS) {
		wiFiPostDataStatus = HTTP_POST_DATA_ERROR_WIFI_NOT_CONNECTED;
		return;
	}
	wiFiPostDataStatus = HTTP_POST_DATA_RUNNING;
	if(xTaskCreate(&restPostStreamFlashTask, "restPostStreamFlashTask", taskStackSize, globalPostParameters, 5, &RTOStaskHandle) != pdPASS) { // IMPORTANT: globalPostParameters and all data within NEEDS TO BE GLOBAL!
		wiFiPostDataStatus = HTTP_POST_DATA_ERROR_TASK_CREATE;
	}
}

void WildFiTagREV5::doWiFiPOSTStreamCallFlashFullBlock(post_task_stream_flash_parameters_t *globalPostParameters, const uint16_t taskStackSize) {
	if(wiFiConnected != WIFI_CONNECT_SUCCESS) {
		wiFiPostDataStatus = HTTP_POST_DATA_ERROR_WIFI_NOT_CONNECTED;
		return;
	}
...
void WildFiTagREV5::killPOSTTask() {
...
bool WildFiTagREV5::getNTPTimestampUTC(bool storeInRTC, uint32_t &timestampUTC, uint16_t &milliseconds, const uint32_t timeoutMs, const char* serverAddr) {
	const uint32_t WAIT_TIME_NTP_DELAY = 20;
	ntpSuccess = false;
	if(wiFiConnected != WIFI_CONNECT_SUCCESS) {
		return false;
	}
	...
	if(ntpSuccess) {
		...
		if(!storeInRTC) {
			...
			if((timestampUTC > 1600000000) && (milliseconds < 1000)) { // looks like a valid timestamp from NTP
				...
				if(!rtc.setTimestamp(timestampUTC + 1)) { return false; }
...
void WildFiTagREV5::printOwnBLEAddress() {
	printf("BLE ADDR: %llX\n", myBLEAddress);
}
...
bool WildFiTagREV5::startBLE(ble_mode_t mode, uint8_t *data, uint8_t len) {
...
bool WildFiTagREV5::checkBLEOkay(uint16_t index) {
...
uint16_t WildFiTagREV5::getBLEBiologgerFound() {
	return bleRXBuffer.biologgerFound;
}

int32_t WildFiTagREV5::getBLERSSISum(uint16_t index) {
...
int8_t WildFiTagREV5::getBLERSSIAverage(uint16_t index) {
	if(checkBLEOkay(index)) {
...
uint16_t WildFiTagREV5::getBLEBeaconCnt(uint16_t index) {
	if(checkBLEOkay(index)) {
...
/*uint64_t WildFiTagREV5::getBLEMacAddress(uint16_t index) {
	if(checkBLEOkay(index)) {
...
uint8_t WildFiTagREV5::getBLEPayloadLen(uint16_t index) {
	if(checkBLEOkay(index)) {
...
uint8_t WildFiTagREV5::getBLEPayload(uint16_t deviceIndex, uint8_t byteIndex) {
...
uint16_t WildFiTagREV5::getBLEBiologgerId(uint16_t index) {
	if(checkBLEOkay(index)) {
...
bool WildFiTagREV5::magnetometerCalibrationMode(uint16_t durationSeconds, mag_calibration_t *calibrationData, bmm150_trim_registers *trimDataIn) {
...
void WildFiTagREV5::customWakeStubFunction(bool (*f)()) {
	wakeStubFunctionPointer = f;
	useCustomWakeStubFunction = true;
}

void WildFiTagREV5::disableWakeStubNoBootIfVoltageLow() {
	wakeStubNoBootIfVoltageLow = false;
}

void WildFiTagREV5::setWakeStubRejectionInterruptSrc(uint8_t interruptSrc) {
