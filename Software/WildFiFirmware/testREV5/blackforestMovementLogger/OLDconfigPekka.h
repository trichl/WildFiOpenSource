...
#define TRACKING_DATA_MODE                              TRACKING_DATA_MODE_1HZ_GPS_AND_ACC      // select tracking firmware: TRACKING_DATA_MODE_1HZ_GPS_AND_ACC or TRACKING_DATA_MODE_ACC_ONLY
...
#define TRACKING_DATA_MODE_1HZ_NO_TIME_TIMEOUT_SECONDS  300                             // when skip getting time: timeout when first trying to get time (before that night time not possible)
#define TRACKING_DATA_MODE_1HZ_NO_TIME_SLEEP_SECONDS    900                             // did not get very first time -> sleep for that amount until trying it again to not drain the battery
#define TRACKING_DATA_MODE_1HZ_GPS_NOT_RESPONDING       3600                            // if GPS detached during operation this will define the time until tag tries again to start GPS
...
#define NIGHTTIME_TURN_ON_HOUR                          11                              // start again logging at this UTC hour
#define NIGHTTIME_TURN_ON_MINUTE                        0                               // start again logging at this UTC minute
#define NIGHTTIME_TURN_OFF_HOUR                         11                              // stop logging at this UTC hour
#define NIGHTTIME_TURN_OFF_MINUTE                       30                              // stop logging at this UTC minute
// ONLY FOR NIGHTTIME_MODE_TRY_DATATRANS
const uint8_t NIGHT_TIME_DATA_TRANS_DEEP_NIGHT_HOURS[11] = { 20, 21, 22, 23, 0, 1, 2, 3, 4, 5, 6 }; // hours when not trying to establish data connection
#define NIGHTTIME_MODE_TRY_DATATRANS_WAKEUP_SECONDS     90                              // in seconds, ESPNOW_CUSTOM_RF_CALIBRATION = true and GPS backup current: 900 = every 15min: 28uA, 300 = every 5min: 48.4uA
#define NIGHTTIME_MODE_TRY_DATATRANS_MODULO             3                               // only every X times of wake ups (NIGHTTIME_MODE_TRY_DATATRANS_EVERY_SECONDS) try data transmission, otherwise just check if night time is over
...
#define POWER_DOWN_MODE_DATA_TRANS_WHEN_VOLTAGE_HALF_OK true                            // try a data transmission if voltage higher than BATT_MIN_VOLTAGE + ((BATT_RESTART_VOLTAGE - BATT_MIN_VOLTAGE) / 2)
...
#define FIRST_UNDER_VOLTAGE_SLEEP_TIME                  3600                            // 1*3600, first time UV detected -> stop imu fifo and sleep for that time
#define FOLLOWING_UNDER_VOLTAGE_SLEEP_TIME              9*3600                          // 12*3600, second and more times UV detected -> sleep for that time

/** Time over WIFI or GPS Configuration (immediately after wake-up) on channel 6, then 1, then 11, each for 120ms */
#define SKIP_GET_TIME                                   true                            // starting without any time (get it later over GPS)
// ONLY IF NOT SKIP_GET_TIME
#define USE_GPS_TO_GET_TIME_AT_BEGINNING                true                            // use not only WiFi but also GPS for getting the time at the beginning
#define USE_GPS_TO_GET_TIME_TIMEOUT_SECONDS             300                             // timeout to only get time at very first start
const uint8_t TIME_WIFI_LIST_SIZE =                     3;                              // should include time wifi
const char* TIME_WIFI_SSIDS[TIME_WIFI_LIST_SIZE] =      { FORCE_DOWNLOAD_WIFI_SSID, "mpitime", "RodelbahnSoelden1" };        // wifi name to scan for at beginning -> if found -> connect, get time, stark tracking
...
/** Activation */
#define ACTIVATION_MODE                                 ACTIVATION_MODE_ON_EVERY_START  // ACTIVATION_MODE_SKIP or ACTIVATION_MODE_STORE_PERMANENTLY or ACTIVATION_MODE_ON_EVERY_START: how shall the tag be activated
...
#define ST_MEMFULL_RESTART_WHEN_MEMORY_FREE_BYTES       2048*64*60                      // only exiting state when at least that memory is free -> 2048*64*2048 = until memory totally free except the one block that is not completely full (2048*64*2048 will never be reached actually - minBlocksToTransmit will be 0)
...
#define COMMAND_BYTE_FORCE_TRACKING_DURATION_SECONDS    600                             // around 88 seconds @12.5Hz until this threshold will be checked
#define COMMAND_BYTE_FORCE_TRACKING_MAX                 15                              // this is the maximum amount of times a forced tracking can be issued per day
