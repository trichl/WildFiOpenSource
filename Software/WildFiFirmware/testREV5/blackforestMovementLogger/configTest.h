...
#define WILDFI_CONFIG_NAME                              "TESTCONFIG"
...
#define TRACKING_DATA_MODE_1HZ_NO_TIME_SLEEP_SECONDS    600                             // did not get very first time -> sleep for that amount until trying it again to not drain the battery
...
#define NIGHTTIME_TURN_OFF_HOUR                         19                              // stop logging at this UTC hour
...
#define NIGHTTIME_MODE_TRY_DATATRANS_WAKEUP_SECONDS     90                              // in seconds, MAX 4096, ESPNOW_CUSTOM_RF_CALIBRATION = true and GPS backup current: 900 = every 15min: 28uA, 300 = every 5min: 48.4uA
...
#define FIRST_UNDER_VOLTAGE_SLEEP_TIME                  10                              // 1*3600, first time UV detected -> stop imu fifo and sleep for that time
...
#define ACTIVATION_BY_ESPNOW_RETRY_TIME_SECONDS         60                             // ESPNOW_CUSTOM_RF_CALIBRATION = true: 300 = every 5min: 44.6uA, 900 = every 15min: 27.5uA -> ESPNOW_CUSTOM_RF_CALIBRATION = false: 300 = every 5min: 109uA, 900 = every 15min: 48.6uA
