...
bool configIsPlausible(tag_config_t *config, uint8_t *errorIdIn) {
    bool allGood = true;
    uint8_t errorId = 0;
    if((config->trackerMode != MODE_TESTRUN) && (config->trackerMode != MODE_PRODUCTIVE) && (config->trackerMode != MODE_SELFTEST)) { allGood = false; errorId = 1; }
    ...
    if((config->gyroFrequency < BMX160_GYRO_ODR_25HZ) || (config->gyroFrequency > BMX160_GYRO_ODR_3200HZ)) { allGood = false; errorId = 111; }
    ...
    if(config->proximityListeningIntervalMs == 0)  { allGood = false; errorId = 21; }
