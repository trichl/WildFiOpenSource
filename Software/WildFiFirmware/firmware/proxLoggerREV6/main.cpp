...
// next free error id = 189

/* TESTING */
// * TEST: 9-axis IMU burst multiple times
// TEST MORE: handle memory full / automatically freeing up memory when full -> ERASE if memory full
// TEST MORE: blob sending function, especially with memory wrap around now
// TEST MORE: checkIfTimeCorrectionNeeded
// TEST MORE: PROXIMITY_COMMAND_FULL_RESET
// TEST: configuration parameters
// TEST: new activation mode position (always enter activation mode)
// TEST: configuration by ESP NOW -> parameter als gateway nachricht
// TEST: detect if BME connected or not (environmentSensorConnected)
// TEST: GPS randomization if night time configured?
// *** FIX: Fix after .000 went to LAT/LNG = 0 once -> debug and re-test

/* HIGHER PRIORITY */
// TODO: REMOVE DEBUG IN GPS FUNCTION (gpsUart output)
...
// WARNING: GPS time accuracy can be horrible for first fixes -> wait at least some seconds to get a good accuracy for time
...
void readFifoIMU(uint8_t *data, uint16_t len) {
    if(!device.imu.readGeneralFIFOInOneGoFast(true, (config.imuMode == IMU_ACC_MAG_GYRO), (config.imuMode == IMU_ACC_MAG_GYRO), data, len, false)) { lastErrorId = 42; errorCnt++; }
        ...
        if(config.imuMode == IMU_ACC_MAG_GYRO) {
            ...
            if(!device.imu.magCompensateFifoData(data, len, datasetStructure, &trimData)) { lastErrorId = 139; errorCnt++;  }
        }
    }
}
...
void checkIfTimeCorrectionNeeded() {
    ...
    int16_t othersBestLastSyncTimeDiff = (scanResults[0].timeDifferenceSumMs / scanResults[0].receivedMessages);
    ...
    if(config.trackerMode == MODE_TESTRUN) { printf("TIMECORR: othersBestLastSync %d (me: %d), othersBestLastSyncTimeDiff %d\n", othersBestLastSync, timestampLastSync, othersBestLastSyncTimeDiff); }

    // TODO: only if timestampLastSync is some time ago (get timestamp) -> maybe not, because might be horribly wrong
    int16_t minDifference = config.timeCorrectionDiffMs; // casting to signed integer
    if(othersBestLastSync > timestampLastSync) { // other tag was synchronized more recently
        if((othersBestLastSyncTimeDiff > minDifference) || (othersBestLastSyncTimeDiff < -minDifference)) { // and time difference is higher
            ...
            if((milliseconds < 1000) && (milliseconds != 0) && (milliseconds > -1000)) {
                ...
                if(config.trackerMode == MODE_TESTRUN) { printf("TIMECORR: done!\n"); }
            }
        }
    }
}
...
/*void wifiPromiscuousActivation(void* buffer, wifi_promiscuous_pkt_type_t type) {
    wifi_promiscuous_pkt_t* p = (wifi_promiscuous_pkt_t*) (buffer);
    if(type == WIFI_PKT_MGMT) { // all esp now messages are MGMT frames
        if(p->rx_ctrl.sig_len == ESPNOW_FRAME_LENGTH_WITHOUT_PAYLOAD + GATEWAY_AROUND_LEN) { // gateway around msg is 8 bytes long
            if(p->rx_ctrl.rate == config.proximityDatarate) { // using the correct proximity data rate
                if(p->payload[ESPNOW_FRAME_OFFSET_VENDOR_SPEC_TYPE] == 0x04) { // is ESP NOW frame
                    if((p->payload[ESPNOW_FRAME_OFFSET_PAYLOAD + ALL_MSGS_PAYLOAD_OFFSET_MSG_TYPE] == ALL_MSGS_PAYLOAD_OFFSET_MSG_TYPE_GATEWAY_AROUND)) { // gateway around message
...
bool getTimeOverWiFiNew() {
    ...
    if(device.initWiFi()) {
        ...
        if(device.scanForWiFisOn1and6and11and13WithPriority((config.trackerMode == MODE_TESTRUN), TIME_WIFI_SSIDS, TIME_WIFI_LIST_SIZE, &foundArrayId, &foundOnChannel, config.timeWifiOutputPower, 120, 500)) { 
...
bool isAnyGatewayInScanResults() {
    for(uint16_t i=0; i<scanResultsPointer; i++) {
...
void activationDetection() {
    ...
    if(nearestGatewayCommand == PROXIMITY_COMMAND_ACTIVATE) {
...
push_data_result_t pushDataIntoMemory(uint8_t *newData, uint32_t newDataLen, bool *memoryFull, bool debug) { // no limit for pushing data
	...
	if(nvsDataLen + newDataLen < NVS_FLASH_BUFFER_SIZE) { // if new data fits into NVS (512 never reached)
		...
		if(freeSpaceInFlash <= dataToStoreInFlash) { // IMPORTANT: if "<" and just fits in memory -> wrap around of pointers to 0.0, then overwriting existing data! horror!
            ...
            *memoryFull = true;			
			...
			// FIRST: write 512 bytes of NVS + newData[part1]
            ...
            // TEST: CONSIDER ERASED DATA
	    	writeStatus = device.flash.fifoPushSimple(flashBlockDeletedPointer, flashPageWritePointer, flashPageWriteOffsetPointer, nvsData, NVS_FLASH_BUFFER_SIZE, true, MOCK_FLASH_WRITES);
            ...
            // SECOND: check if more than 512 bytes left (then directly write to flash)
            ...
            if(leftDataToWrite >= NVS_FLASH_BUFFER_SIZE) {
                ...
                // TEST: CONSIDER ERASED DATA
	 	    	writeStatus = device.flash.fifoPushSimple(flashBlockDeletedPointer, flashPageWritePointer, flashPageWriteOffsetPointer, newData + newDataLeftPointer, flashWriteLength, true, MOCK_FLASH_WRITES);
		    	if(writeStatus == MT29_SEQ_WRITE_STATUS_MEMORY_FULL) { returnVal = PUSH_DATA_MT29_SEQ_WRITE_STATUS_MEM_FULL_FATAL; } // SHOULD NOT HAPPEN, checked before, don't stop in case of error
	            else if(writeStatus == MT29_SEQ_WRITE_STATUS_BUFFER_ERROR) { returnVal = PUSH_DATA_MT29_SEQ_WRITE_STATUS_BUFFER_ERROR; } // don't stop in case of error
	            else if(writeStatus == MT29_SEQ_WRITE_STATUS_PARTIAL_WRITE_ERROR) { returnVal = PUSH_DATA_MT29_SEQ_WRITE_STATUS_PARTIAL_WRITE_ERROR; } // don't stop in case of error
	            else if(writeStatus == MT29_SEQ_WRITE_STATUS_READ_BACK_ERROR) { returnVal = PUSH_DATA_MT29_SEQ_WRITE_STATUS_READ_BACK_ERROR; } // don't stop in case of error
...
void storeGPSDataInFlash(uint32_t timestamp, esp_gps_t *gpsData, bool *memoryFull) {
    ...
    if(!device.flash.createBuffer(&sensorData, sensorDataLen)) { lastErrorId = 24; errorCnt++; return; }
        ...
        if(res != PUSH_DATA_SUCCESS) { lastErrorId = 93 + res; errorCnt++; } // 15 error codes (0 = sucess), 94 - 108
...
void storeProximityDataInFlash(uint32_t timestamp, uint8_t imuMode, uint8_t *fifoData, uint16_t fifoLen, bool withEnv, int16_t temperature, uint32_t pressure, uint32_t humidity, bool *memoryFull) {
    ...
    if(!device.flash.createBuffer(&sensorData, headerDataLen + proxDataLen + fifoLen)) { lastErrorId = 34; errorCnt++; return; }
        ...
        if(res != PUSH_DATA_SUCCESS) { lastErrorId = 79 + res; errorCnt++; } // 15 error codes (0 = sucess) -> 80 - 94
        heap_caps_free(sensorData); 
    }
}
...
void imuLightSleepTrackingMode(bool baroAlreadyInitialized, uint32_t burstDurationMilliseconds) { // IMU needs to be on, flash needs to be off
    ...
    // initialize fifo interrupts
    if(!device.imu.enableFIFOInterrupt(980)) { lastErrorId = 148; errorCnt++; return; }
    ...
    while(keepTrackingRunning) {
        ...
        if(voltage < config.battMinVoltage) { // check voltage
            ...
            if(((fifoDataPointer + 1024 + imuLightSleepTrackingModeHeaderLength) >= FIFO_DATA_LEN) // no more space in RAM memory after next light sleep
                ...
                if(!device.flashPowerOn(false)) { lastErrorId = 154; errorCnt++; } // turn on flash power already
                ...
                if(res != PUSH_DATA_SUCCESS) { lastErrorId = 154 + res; errorCnt++; } // 15 error codes (0 = sucess), 155 - 170
    ...
    while(1) {
        if((config.trackerMode == MODE_TESTRUN) || (config.trackerMode == MODE_PRODUCTIVE)) {
            ...
            /** ---------------- SERIAL MENUE STATE ---------------- */
                ...
                if(!device.serialMenue(true, NVS_OWN_ID, NVS_FLASH_TAG_ACTIVATED_BY_WIFI, gpsCheckAliveAndMaybeChangeBaudrateWrapper)) { // WARNING: CPU clocked to 10 MHz
            ...
            /** ---------------- FIRST START STATE ---------------- */
                ...
                gps.checkAliveAndMaybeChangeBaudrate(&device, config.trackerMode == MODE_TESTRUN);
                ...
                if(config.imuMode == IMU_ACC_MAG_GYRO) { fillIMUTrimData(); }
                ...
                // go into activation state for quick scan
                state = ST_ACTIVATION;
                esp_sleep_enable_timer_wakeup(100000ULL); // 100ms
            }
            /** ---------------- WAIT FOR ACTIVATION STATE ---------------- */
                ...
                if(device.readSupplyVoltageFromWakeStub() < config.battMinVoltage) { // voltage low, stay in that mode but sleep for an hour or more -> needs to be here because otherwise setHourlyInterrupt might not have been set!
                    ...
                    else if(nearestGatewayCommand == PROXIMITY_COMMAND_ACTIVATE) { 
            ...
            /** ---------------- GET TIME STATE ---------------- */
                ...
                if(device.readSupplyVoltageFromWakeStub() < config.battMinVoltage) { // voltage low, stay here, but sleep long
                    ...
                    if(hasValidTimestamp) {
                        ...
                        device.enableInternalTimerInterruptInDeepSleep(1); // restart in x seconds and move to activation or start
                    }
                    else {
            ...
            /** ---------------- TRACKING STATE ---------------- */
                ...
                if(device.readSupplyVoltageFromWakeStub() < config.battMinVoltage) {
                    ...
                    bool proximitySuccess = proximityDetection(&timestamp); // perform proximity detection and return timestamp
                    scanStop = true; // do not store more incoming messages in scanResults (ESP NOW still running)
                    bool gatewaySeen = isAnyGatewayInScanResults(); // check if any gateway is in the scan results
                    bool memoryFull = false;
                    ...
                    if(!proximitySuccess) { // some fatal error
                        ...
                        checkForHugeTimeDifferences(); // will just set an error
                        ...
                        if(useEnvironmentSensor()) {
                            if(device.baro.getResults()) { // normally waits up to 31ms, but measurement was triggered before during proximity detection
                                ...
                                if(config.trackerMode == MODE_TESTRUN) { printf("ENV: %d deg, %d press, %d hum\n", temperature, pressure, humidity); }
                            }
                        }
                        ...
                        if((config.imuMode == IMU_ACC_ONLY) || (config.imuMode == IMU_ACC_MAG_GYRO)) {
                            /*if(IMU_GET_FULL_BURSTS_CNT > 0) {
                                if(!gatewaySeen) { // WARNING: only possible when no gateway seen (no data transmission), otherwise RF is still on and can't use light sleep
                                    if(!device.imu.enableFIFOInterrupt(980)) { lastErrorId = 130; errorCnt++; }
                                    device.enableAccInterruptInDeepSleep();
                    ...
                    bool justSynchronizedRTCViaGPS = false; // to not re-do RTC sync between tags after GPS synchronization
                    ...
                    bool fullResetCommanded = false;
                    bool deactivationCommanded = false;
                    if(nearestGatewayCommand == PROXIMITY_COMMAND_CHANGE_CONFIG) {
                        bool changedSomething = false;
                        if(!checkIfReconfigNeeded(&device, &config, nearestGatewayConfiguration, nearestGatewayCommand, nearestGatewayRssi, &changedSomething, (config.trackerMode == MODE_TESTRUN))) { lastErrorId = 184; errorCnt++; }
                        if(changedSomething) { device.blinkTimes(12, B_BOTH); }
