...
#define WILDFI_CONFIG_NAME                              "JENSI"
#define WILDFI_CONFIG_VERSION                           10

/** IMPORTANT settings */                      
#define TRACKER_MODE                                    MODE_PRODUCTIVE                 // MODE_ONE_TIME_PROGRAMMING or MODE_TESTRUN or MODE_PRODUCTIVE or MODE_SELFTEST
...
#define GPS_FORCED_AFTER_EVERY_PROXIMITY                false                           // ALWAYS execute GPS after proximity detection, gpsFixHours still used
...
const char* TIME_WIFI_SSIDS[TIME_WIFI_LIST_SIZE] =      { "RodelbahnSoelden", "wildfi" };   // wifi name to scan for at beginning -> if found -> connect, get time, stark tracking
const char* TIME_WIFI_PASSWORDS[TIME_WIFI_LIST_SIZE] =  { "xxxxxx", "wildfi01" };        // wifi password for activation/timestamp                  
#define TIME_WIFI_OUTPUT_POWER                          RADIO_MAX_TX_11_DBM                 // 19.5dBm will brown out on REV3
#define TIME_BETWEEN_GET_TIME_RETRIES_SECONDS           600                                 // 120 = 660uA, if not activated: sleep for that time, wake up, check if wifi there and try to activate, 120 = 234uA average
