...
bool isTimeToGoToBed() {
    ...
    if(NIGHTTIME_ENTER == NIGHTTIME_ALWAYS_NIGHT) { return true; } // always time to go to bed (only force mode possible)
    else if(NIGHTTIME_ENTER == NIGHTTIME_DISABLED) { return false; } // never time to go to bed
    currentHour = device.rtc.getHours(error);
...
void wifiPromiscuousGatewayAround(void* buffer, wifi_promiscuous_pkt_type_t type) {
    ...
    if(!gatewayAroundMessageEspNowReceived) { // TODO: NEW, TEST
...
uint16_t imuAnd1HzGPSTrackingModeHeaderLength() {
    return 32;
}
...
void imuAnd1HzGPSTrackingModeAddData(esp_gps_t *gpsData, uint16_t currentFifoLen, uint8_t *buffer, uint16_t *bufferPointer) {
    ...
    if((USE_MAGNETOMETER) && (USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x5A, buffer, bufferPointer); }
    else if((!USE_MAGNETOMETER) && (USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x5B, buffer, bufferPointer); }
    else if((USE_MAGNETOMETER) && (!USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x5C, buffer, bufferPointer); }
    else if((!USE_MAGNETOMETER) && (!USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x5D, buffer, bufferPointer); }
        ...
        if(commandByte == COMMAND_BYTE_DEACTIVATE) {
            ...
            while(keepTrackingRunning) {
                ...
                if(voltage < BATT_MIN_VOLTAGE) {
                    ...
                    if((fifoDataPointer + currentFifoLen + imuAnd1HzGPSTrackingModeHeaderLength()) <= FIFO_DATA_LEN) {
                        // add header data
                        uint16_t fifoDataPointerOld = fifoDataPointer;
                        imuAnd1HzGPSTrackingModeAddData(&gpsData, currentFifoLen, fifoData, &fifoDataPointer);
                        ...
                        imuAnd1HzGPSTrackingModeAddData(&gpsData, currentFifoLen, fifoDataNewest, &fifoDataNewestPointer);
...
void imuLightSleepTrackingModeAddData(uint16_t voltage, uint16_t currentFifoLen, uint8_t *buffer, uint16_t *bufferPointer) { 
    ...
    HelperBits::addData1_AndIncrementPointer(0x12, buffer, bufferPointer);
    HelperBits::addData1_AndIncrementPointer(0x34, buffer, bufferPointer);
    if((USE_MAGNETOMETER) && (USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x5E, buffer, bufferPointer); }
    else if((!USE_MAGNETOMETER) && (USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x5F, buffer, bufferPointer); }
    else if((USE_MAGNETOMETER) && (!USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x60, buffer, bufferPointer); }
    else if((!USE_MAGNETOMETER) && (!USE_GYRO)) { HelperBits::addData1_AndIncrementPointer(0x61, buffer, bufferPointer); }
    HelperBits::addData4_AndIncrementPointer(timestamp, buffer, bufferPointer);
    HelperBits::addData1_AndIncrementPointer(milliseconds, buffer, bufferPointer);
    HelperBits::addData1_AndIncrementPointer(lastErrorId, buffer, bufferPointer);
    ...
    while(1) {
        if((TRACKER_MODE == MODE_TESTRUN) || (TRACKER_MODE == MODE_PRODUCTIVE)) {
            ...
            /** ---------------- START STATE ---------------- */
                ...
                if(TRACKING_DATA_MODE == TRACKING_DATA_MODE_IMU_SD_DEEP_SLEEP) { device.enableAccInterruptInDeepSleep(); }
                else if(TRACKING_DATA_MODE == TRACKING_DATA_MODE_1HZ_GPS_AND_IMU) { device.enableInternalTimerInterruptInDeepSleep(1); }
            ...
            /** ---------------- TRACKING STATE ---------------- */
                ...
                if(TRACKING_DATA_MODE == TRACKING_DATA_MODE_IMU_SD_DEEP_SLEEP)          { imuDeepSleepTrackingMode(); }
                else if(TRACKING_DATA_MODE == TRACKING_DATA_MODE_1HZ_GPS_AND_IMU)       { imuAnd1HzGPSTrackingMode(); }
                else if(TRACKING_DATA_MODE == TRACKING_DATA_MODE_IMU_HD_LIGHT_SLEEP)    { imuLightSleepTrackingMode(); }
            }
                    ...
                    /** OPTION 2: transmit data every now and then, even in sleep */
                        ...
                        if(device.readSupplyVoltageFromWakeStub() < BATT_MIN_VOLTAGE) { // voltage low, go to power down
                            ...
                            if(commandByte == COMMAND_BYTE_FORCE_TRACKING) {
                                ...
                                if((espNowForceTrackingOnceCnt < COMMAND_BYTE_FORCE_TRACKING_MAX) || (NIGHTTIME_ENTER == NIGHTTIME_ALWAYS_NIGHT)) { // if only night mode the counter would never get reset
                                    espNowForceTrackingOnceCnt++;
