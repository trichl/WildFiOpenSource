...
#define WILDFI_CONFIG_NAME                              "CESSNA"
...
#define TRACKING_DATA_MODE                              TRACKING_DATA_MODE_IMU_HD_LIGHT_SLEEP // select tracking firmware: TRACKING_DATA_MODE_1HZ_GPS_AND_ACC or TRACKING_DATA_MODE_ACC_ONLY or TRACKING_DATA_MODE_IMU_HD_LIGHT_SLEEP
#define DO_THE_BLINK                                    true

/** Data transmission to use */
#define TRANSMISSION_METHOD                             TRANSMISSION_METHOD_ESP_NOW     // how to transmit data, TRANSMISSION_METHOD_ESP_NOW or TRANSMISSION_METHOD_WIFI

/** TRACKING_DATA_MODE_1HZ_GPS_AND_ACC settings */
#define TRACKING_DATA_MODE_1HZ_NO_TIME_TIMEOUT_SECONDS  300                             // 300, when skip getting time: timeout when first trying to get time (before that night time not possible)
#define TRACKING_DATA_MODE_1HZ_NO_TIME_SLEEP_SECONDS    3600                            // 3600, did not get very first time -> sleep for that amount until trying it again to not drain the battery
...
#define TRACKING_DATA_MODE_1HZ_GPS_BLINK                false                           // only valid if DO_THE_BLINK = true, blink shortly (red = no fix, green = got fix) if GPS is active
...
acc_config_t accConfig = {
                                                        ...
                                                        BMX160_ACCEL_RANGE_4G           // BMX160_ACCEL_RANGE_8G, acc range (WARNING: changes meaning of LSB value in data)
};

#define USE_MAGNETOMETER                                false
mag_config_t magConfig = {                              BMX160_MAG_ODR_50HZ,
                                                        BMX160_MAG_ACCURACY_REGULAR
};

#define USE_GYRO                                        false
gyro_config_t gyroConfig = {                            BMX160_GYRO_ODR_50HZ,
...
#define NIGHTTIME_TURN_ON_HOUR                          4                               // start again logging at this UTC hour
...
const uint8_t NIGHT_TIME_DATA_TRANS_DEEP_NIGHT_HOURS[3] = { 1, 2, 3 }; // hours when not trying to establish data connection
#define NIGHTTIME_MODE_TRY_DATATRANS_WAKEUP_SECONDS     90                              // 180, in seconds, MAX 4096, ESPNOW_CUSTOM_RF_CALIBRATION = true and GPS backup current: 900 = every 15min: 28uA, 300 = every 5min: 48.4uA
...
#define SELFTEST_GPS_SET_BAUDRATE_PERMANTENLY           false                           // trying to set baud rate of GPS to 115200 permanently

/** Undervoltage mode: battery protection */
#define POWER_DOWN_MODE_DATA_TRANS_WHEN_VOLTAGE_HALF_OK false                           // true, try a data transmission if voltage higher than BATT_MIN_VOLTAGE + ((BATT_RESTART_VOLTAGE - BATT_MIN_VOLTAGE) / 2)
...
const char* TIME_WIFI_SSIDS[TIME_WIFI_LIST_SIZE] =      { FORCE_DOWNLOAD_WIFI_SSID, "wildfi", "RodelbahnSoelden" }; // wifi name to scan for at beginning -> if found -> connect, get time, stark tracking
const char* TIME_WIFI_PASSWORDS[TIME_WIFI_LIST_SIZE] =  { FORCE_DOWNLOAD_WIFI_PASSWORD, "wildfi01", "xxxxxx" }; // wifi password for activation/timestamp                  
#define TIME_WIFI_OUTPUT_POWER                          RADIO_MAX_TX_11_DBM             // 19.5dBm will brown out on REV3
#define TIME_BETWEEN_GET_TIME_RETRIES_SECONDS           180                             // 3600, 120 = 660uA, if not activated: sleep for that time, wake up, check if wifi there and try to activate, 120 = 234uA average
...
#define ACTIVATION_BY_ESPNOW_RETRY_TIME_SECONDS         300                             // ESPNOW_CUSTOM_RF_CALIBRATION = true: 300 = every 5min: 44.6uA, 900 = every 15min: 27.5uA -> ESPNOW_CUSTOM_RF_CALIBRATION = false: 300 = every 5min: 109uA, 900 = every 15min: 48.6uA

/** Configuration Data Transmission: Common */
#define DATATRANSM_TRY_EVERY_FULL_MINUTE_SELDOMLY       2                               // 12*60 = 00:00 and 12:00 (UTC), 2 = on every even minute, 15 = 14:00, 14:15, .., only IF DATA TO TRANSMIT (see MIN_BLOCKS_TO_TRANSMIT): last data transmission nothing transmitted -> re-try more seldomly
...
#define ST_MEMFULL_TRY_EVERY_FULL_MINUTE_SELDOMLY       15                              // 1*60, 12*60 = 00:00 and 12:00 (UTC), 2 = on every even minute, 15 = 14:00, 14:15, .., only IF DATA TO TRANSMIT (see MIN_BLOCKS_TO_TRANSMIT): last data transmission nothing transmitted -> re-try more seldomly
...
#define ACC_INTERRUPT_WATERMARK                         900                             // fine for <= 50Hz, other stuff not tested, imu is generating interrupt when fifo is at that fill level
...
#define ACC_RAM_SIZE_2                                  1                               // see above
...
#define COMMAND_BYTE_FORCE_TRACKING_DURATION_SECONDS    1800                            // 1800, around 88 seconds @12.5Hz until this threshold will be checked
#define COMMAND_BYTE_FORCE_TRACKING_MAX                 5                               // this is the maximum amount of times a forced tracking can be issued per day

#if TRANSMISSION_METHOD == TRANSMISSION_METHOD_ESP_NOW
#define ESPNOW_OUTPUT_POWER                             RADIO_MAX_TX_11_DBM             // RADIO_MAX_TX_19_5_DBM, 19.5dBm will brown out on REV3
